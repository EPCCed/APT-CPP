...
Typically a trait is a template `struct` that accepts one or more types as
parameter(s) and either:

.columns[
.col2[
Tells you something about the type(s) by producing a value, which is
stored in the `value` static member variable, e.g.
...
If not you can investigate template specialisation, which is beyond
the scope of this course.

---
# Exercise

Please try the second part of [exercises/morton-order](../../exercises/morton-order).
