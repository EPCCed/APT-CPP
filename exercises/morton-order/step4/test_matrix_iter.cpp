...
template <typename T>
morton::matrix<T> make_filled(int N) {
  morton::matrix<T> mat(N);
...
bool test_mut_iter() {
  constexpr int N = 4;
  auto mat = make_filled<unsigned>(N);
  return check_mat<N>(mat);
}

bool test_rev_iter() {
  const int N = 4;
  auto mat = make_filled<char>(N);
...
// Basic test of const iterator
bool test_const_iter() {
  const int N = 4;
  const morton::matrix<int>& mat = make_filled<int>(N);
  return check_mat<N>(mat);
}
