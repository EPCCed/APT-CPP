...
template <typename T>
morton::matrix<T> make_diag(uint32_t rank) {
  auto mat = morton::matrix<T>{rank};
  // fill diagonal
...
bool test_move() {
  auto m1 = make_diag<char>(4);
  auto m2 = make_diag<char>(8);
  ...
  const morton::matrix<char>& matref = (m1 = std::move(m2));
...
int main() {
  static_assert(!std::is_copy_constructible<morton::matrix<int>>::value,
		"Require that morton matrix is not copyable");
  static_assert(std::is_move_constructible<morton::matrix<int>>::value,
