...
morton::matrix make_diag(uint32_t rank) {
  auto mat = morton::matrix{rank};
  // fill diagonal
...
bool test_move() {
  ...
  const morton::matrix& matref = (m1 = std::move(m2));
...
int main() {
  static_assert(!std::is_copy_constructible<morton::matrix>::value,
		"Require that morton matrix is not copyable");
  static_assert(std::is_move_constructible<morton::matrix>::value,
